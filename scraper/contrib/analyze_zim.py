...
def analyze_zim(path: str):
    ...
    for playlist in data.get("playlists", []):
        for v in playlist.get("videos", []):
            ...
            for ext in ("webm", "mp4"):
                try:
                    size = archive.get_entry_by_path(f"videos/{vid}/video.{ext}").get_item().size
                    break
                except KeyError:
                    pass
            if size and dur > 0:
...
def main():
    ...
    for zim_path in sys.argv[1:]:
        ...
        for slug, dur, size in videos.values():
            ...
            ratio = mb / mins
            ratios.append(ratio)
            print(f"  {slug[:50]:<50s}  {mins:6.1f}min  {mb:7.1f}MB  {ratio:5.2f} MB/min")
    ...
    if all_ratios:
        ...
        print(f"  1 min of YouTube ≈ {avg:.2f} MB in ZIM  (±{std:.2f}, median {median:.2f})")
