...
watch(
  () => main.channel?.userShortUploadsPlaylist,
  () => {
    fetchData()
  }
)
...
const fetchData = async function () {
  if (main.channel?.userShortUploadsPlaylist) {
    try {
      const resp = await main.fetchPlaylist(main.channel?.userShortUploadsPlaylist)
      if (resp) {
        playlist.value = resp
...
<template>
  ...
  <div v-else>
     <tab-info
      :title="'Shorts from '+main.channel?.title"
      :count="playlist?.videosCount || 0"
      :count-text="playlist?.videos.length === 1 ? 'video' : 'videos'"
      icon="mdi-video-outline"
    />
    <video-grid v-if="videos" :videos="videos" :playlist-slug="main.channel?.userShortUploadsPlaylist" />
  </div>
</template>
