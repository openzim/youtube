...
def main():
    parser = argparse.ArgumentParser(
        prog=NAME,
        description="Runs youtube2zim to create one zim per playlist from a channel(s)/user(s)/list of playlists",
    )

    parser.add_argument(
        "--type",
        help="Type of collection",
        choices=[CHANNELS, PLAYLISTS, USERS],
        required=True,
    ...
    parser.add_argument(
        "--output",
        help="Output folder for ZIM files or build folders",
        default="/output",
        dest="output_dir",
    )
    ...
    parser.add_argument(
        "--concurrency",
        help="Number of concurrent threads to use per instance of youtube2zim",
        type=int,
    ...
    try:
        ...
        runner = Youtube2PlaylistZims(**dict(args._get_kwargs()), youtube_store=YOUTUBE)
        runner.run()
